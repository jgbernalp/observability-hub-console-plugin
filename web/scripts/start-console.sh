...
if [[ $BACKEND_HOST == 0 ]]; then
    BACKEND_HOST="$INTERNAL_HOST:9002"
fi
...
function createEnvironment(){
    ...
    BRIDGE_PLUGIN_PROXY="{\"services\": [{\"consoleAPIPath\": \"/api/proxy/plugin/observability-ui-hub/backend/\", \"authorize\": true, \"endpoint\": \"${BACKEND_HOST}\"}]}"
    if [[ $USE_LOCAL_PROXY == 1 ]]; then
        echo "Using local proxy"
        echo BRIDGE_PLUGIN_PROXY=$BRIDGE_PLUGIN_PROXY >> scripts/env.list
    fi

    BRIDGE_PLUGINS="observability-ui-hub=${INTERNAL_HOST}:${PLUGIN_PORT}"
